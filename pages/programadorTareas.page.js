...
export default class ProgramadorTareasPage {
  constructor(driver) {
    this.driver = driver;
  }

  async ejecutarProgramadorTareas() {
    ...
    try {
      ...
      // === Paso 3: Clic en "Programador de tareas" ===
      const targetApp = await driver.wait(
        until.elementLocated(By.xpath("//div[contains(@class,'legend-application') and normalize-space()='Programador de tareas']")),
        10000
      );
      ...
      console.error("❌ Error en Programador de Tareas:", error.message);
      const screenshot = await driver.takeScreenshot();
      const carpetaErrores = path.resolve(__dirname, '../errores');
      if (!fs.existsSync(carpetaErrores)) fs.mkdirSync(carpetaErrores);
      const filePath = path.join(carpetaErrores, `error_programadorTareas_${Date.now()}.png`);
