...
export default class CreacionCitasGestionOrdenVentaPage {
    constructor(driver) {
        this.driver = driver;
    }

    async ejecutarGestionOrden() {
        try { 
            ...
            //     10000
            // );
            // await driver.executeScript("arguments[0].scrollTop = arguments[0].scrollHeight;", contenedorOrdenes);
            // await driver.sleep(2000); // Espera para renderizado

            // // === Paso 2: Hacer clic en la última orden creada ===
            // const ordenes = await contenedorOrdenes.findElements(By.css('div.item-list'));
            // if (ordenes.length === 0) throw new Error("❌ No se encontraron órdenes en el contenedor lateral");
            ...
            // if (!await ultimaOrden.isDisplayed()) throw new Error("❌ La última orden no es visible");
            ...
            // if (!(await botonCentroPoblado.isDisplayed()) || !(await botonCentroPoblado.isEnabled())) {
            ...
            // // === Paso 17: Seleccionar opción en campo CANAL DE VENTA ===

            // // Esperar el <select>
            // const selectCanalVenta = await driver.wait(
            //     until.elementLocated(By.id("input-select-CANALDEVENTA")),
            //     10000
            // );

            // // Obtener todas las opciones válidas, excluyendo el placeholder
            // const opcionesCanalVenta = await selectCanalVenta.findElements(By.css('option:not([value="placeholder"])'));

            // // Validar que existan opciones disponibles
            // if (opcionesCanalVenta.length === 0) {
            //     throw new Error("❌ No hay opciones disponibles en CANAL DE VENTA");
            // }

            // // Elegir una opción aleatoria (nombre único)
            // const opcionCanalVenta = opcionesCanalVenta[Math.floor(Math.random() * opcionesCanalVenta.length)];
            // const valorCanalVenta = await opcionCanalVenta.getAttribute('value');

            // // Seleccionar la opción
            // await selectCanalVenta.sendKeys(valorCanalVenta);

            // // console.log(`✅ CANAL DE VENTA seleccionado: ${valorCanalVenta}`);

            // // Pausa breve opcional
            // await driver.sleep(19000);

            // // === Paso 18: Seleccionar opción en campo MEDIO DE CONTACTO ===

            // // Esperar el <select> del campo MEDIO DE CONTACTO
            // const selectMedioContacto = await driver.wait(
            //     until.elementLocated(By.id("input-select-MEDIODECONTACTO")),
            //     10000
            // );

            // // Obtener todas las opciones válidas, excluyendo el placeholder
            // const opcionesMedioContacto = await selectMedioContacto.findElements(By.css('option:not([value="placeholder"])'));

            // // Validar que existan opciones
            // if (opcionesMedioContacto.length === 0) {
            //     throw new Error("❌ No hay opciones disponibles en MEDIO DE CONTACTO");
            // }

            // // Elegir una opción aleatoria
            // const opcionMedioContacto = opcionesMedioContacto[Math.floor(Math.random() * opcionesMedioContacto.length)];
            // const valorMedioContacto = await opcionMedioContacto.getAttribute('value');

            // // Seleccionar la opción
            // await selectMedioContacto.sendKeys(valorMedioContacto);

            // // console.log(`✅ MEDIO DE CONTACTO seleccionado: ${valorMedioContacto}`);

            // // Pausa breve opcional
            // await driver.sleep(3000);

            // // === Paso 19: Seleccionar opción en campo OPERADOR ACTUAL ===

            // // Esperar el <select> del campo OPERADOR ACTUAL
            // const selectOperadorActual = await driver.wait(
            //     until.elementLocated(By.id("input-select-OPERADORACTUAL")),
            //     10000
            // );

            // // Obtener todas las opciones válidas, excluyendo el placeholder
            // const opcionesOperadorActual = await selectOperadorActual.findElements(By.css('option:not([value="placeholder"])'));

            // // Validar que existan opciones
            // if (opcionesOperadorActual.length === 0) {
            //     throw new Error("❌ No hay opciones disponibles en OPERADOR ACTUAL");
            // }

            // // Elegir una opción aleatoria
            // const opcionOperadorActual = opcionesOperadorActual[Math.floor(Math.random() * opcionesOperadorActual.length)];
            // const valorOperadorActual = await opcionOperadorActual.getAttribute('value');

            // // Seleccionar la opción
            // await selectOperadorActual.sendKeys(valorOperadorActual);

            // //console.log(`✅ OPERADOR ACTUAL seleccionado: ${valorOperadorActual}`);
